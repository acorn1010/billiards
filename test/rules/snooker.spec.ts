...
//     Ball.id = 0;
//     container = new Container(undefined, Assets.localAssets(rule), rule);
...
//     table.balls
//       .filter((b) => b.id > 6)
//       .forEach((b) => (b.state = PoolBallState.InPocket));
//   }
//
...
//
//     // yellow potted
//     table.balls[1].state = PoolBallState.InPocket;
//
//     // pot green
...
//     table.cueball.state = PoolBallState.InPocket;
...
//     table.balls[1].state = PoolBallState.InPocket;
//     table.balls[5].state = PoolBallState.InPocket;
//     snooker.update(outcome);
//     expect(snooker.targetIsRed).to.be.true;
...
//     table.balls[8].state = PoolBallState.InPocket;
//     snooker.update(outcome);
//     expect(snooker.targetIsRed).to.be.true;
//     expect(snooker.foulPoints).to.be.equal(6);
//     done();
...
//     outcome.push(Outcome.collision(table.cueball, table.balls[5], 1));
//     outcome.push(Outcome.pot(table.balls[5], 1));
//     table.balls[6].state = PoolBallState.InPocket;
//     snooker.update(outcome);
//     expect(snooker.targetIsRed).to.be.true;
//     expect(snooker.foulPoints).to.be.equal(6);
...
//     expect(snooker.placeBall(ZERO_VECTOR).length()).to.be.greaterThan(0);
//     done();
//   });
// });
