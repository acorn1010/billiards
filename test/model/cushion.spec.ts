...
import { CushionRigidBody } from "../../src/model/physics/cushion";
...
const cushion = new CushionRigidBody(pocketGeometry);
describe("Cushion", () => {
  ...
  it("bounces off knuckle", (done) => {
    ...
    ball.vel.y = -10 * R;
    expect(cushion.bounceAny(ball, t)).to.be.undefined;
    const k = pocketGeometry.findBouncingKnuckle(ball, t);
    expect(k).to.be.deep.equal(pocketGeometry.getPockets().pocketS.knuckleSE);
    k?.bounce(ball);
    expect(ball.vel.x).to.be.below(0);
    expect(ball.vel.y).to.be.above(0);
    done();
