...
describe("Table", () => {
  ...
  it("updates when single ball stationary", (done) => {
    const table = new Table([new Ball(ZERO_VECTOR)]);
    expect(table.prepareAdvanceAll(t)).to.be.true;
    expect(table.allStationary()).to.be.true;
    done();
  });
  ...
  it("a pots b", (done) => {
    ...
    const maxiter = 10;
    let i = 0;
    while (i++ < maxiter && b.state != PoolBallState.InPocket) {
      table.advance(10 * t);
    }
    expect(b.isFalling()).to.be.false;
    expect(b.state).to.be.equal(PoolBallState.InPocket);
